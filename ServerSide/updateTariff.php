...
$tariff = isset($_POST['tariff']) ? (int)$_POST['tariff'] : null;
...
// Apply tariff change
if($tariff === null) {
    $stmt = $con->prepare("UPDATE `cars` SET `tariff` = NULL WHERE `cars`.`id` = ? AND `cars`.`owner_id` = ?");
    $stmt->bind_param("ii", $car_id, $user_id);
}
else {
    $stmt = $con->prepare("UPDATE `cars` SET `tariff` = ? WHERE `cars`.`id` = ? AND `cars`.`owner_id` = ?");
    $stmt->bind_param("iii", $tariff, $car_id, $user_id);
}
